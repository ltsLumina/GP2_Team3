...
public class DebugWindow : EditorWindow
{
    ...
    static void HelpCommand()
    {
        ...
        else Debug.Log("Commands can be viewed from the commands list at the bottom of the" + nameof(DebugWindow));
    }
    #endregion
    ...
    static void DrawEditorWindowsMenu()
    {
        ...
        using (new VerticalScope("box"))
        {
            ...
            if (windowsFoldout) CreateButtonWithAction("Nothing to Display.", () => Debug.LogWarning("Nothing to display."));
        }
    }
    ...
    static void DrawSearchAndPingAssetMenu()
    {
        ...
        if (Button("Search", Width(100), Height(25)))
        {
            ...
            {
                Debug.Log($"Type a search query to find an asset. \nIf there are too many assets found, the editor will only ping the first {maxPingedAssets} assets.");
                Debug.Log(@"Type \\""\Settings\"" to change the maxPingedAssets.");
                searchQuery = string.Empty;
                return;
            }
            ...
            switch (guids.Length)
            {
                ...
                case > 1:
                    ...
                    switch (pingExact)
                    {
                        ...
                        case 2:
                            Debug.LogWarning("Asset search cancelled.");
                            searchQuery = string.Empty;
                            return;
                    }
        ...
        get
        {
            if (isSettingMaxPingedAssets)
            {
                if (int.TryParse(searchQuery, out int newMax))
                {
                    ...
                    Debug.Log($"Max pinged assets set to {maxPingedAssets}.");
                    searchQuery = string.Empty;
                    return true;
                }
                ...
                if (!isSettingMaxPingedAssets) Debug.LogWarning("Settings command cancelled.");
                return true;
            }
    ...
    static void DrawConsoleCommandMenu()
    {
        ...
        bool CommandExecuted()
        {
            ...
            else Debug.LogWarning("Command is empty.");
    ...
    static void OpenScene(int sceneIndex)
    {
        ...
        else { Debug.LogWarning("Scene Load Cancelled."); }
    }
